...
def trade_stats(trades, base: float = 1000.0, n_accounts: int = 1) -> dict:
    ...
    ``max_dd_pct`` is the worst peak-to-trough of the PnL cumsum relative to the
    total capital in play (``base * n_accounts``). ``profit_factor`` and
    ``expectancy_in_risks`` are ``None`` when there are no losing trades — the
    ratio is undefined, not "999".
    """
    pnls = _pnls(trades)
    n = int(pnls.size)
    empty = dict(
    ...
    max_dd_pct = 100.0 * max_dd_pnl / max(abs(base) * max(int(n_accounts), 1), 1e-9)
